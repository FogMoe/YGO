...
bool Game::Initialize() {
	...
	device->setWindowCaption(L"FOGMOEYGO");
	...
	//main menu
	wchar_t strbuf[256];
	myswprintf(strbuf, L"FOGMOEYGO %X.0%X.%X", PRO_VERSION >> 12, (PRO_VERSION >> 4) & 0xff, PRO_VERSION & 0xf);
...
void Game::MainLoop() {
	...
	while(device->run()) {
		...
		if(cur_time >= 1000) {
			myswprintf(cap, L"FOGMOEYGO FPS: %d", fps);
